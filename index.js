const heroes = [
  ...
  },
  {
    ...
    url: "https://i.pinimg.com/474x/91/08/a8/9108a8266fc5d19659089a2712fb045e.jpg",
  ...
  },
  {
    ...
    url: "https://i.pinimg.com/474x/88/6f/5b/886f5b3a73d5b71ac051d252b8b7fbb9.jpg",
  ...
  },
  {
    ...
    url: "https://i.pinimg.com/474x/6d/48/c9/6d48c918b10fdaf8b8d1624d4d1521aa.jpg",
...
function loadRatings() {
  ...
  heroes.forEach((hero) => {
    if (savedRatings[hero.name]) {
      hero.rating = savedRatings[hero.name];
    } else {
      hero.rating = 0; // если нет сохраненной оценки
    }
  });
}
...
function saveRatings() {
  const ratings = {};
  heroes.forEach((hero) => {
    ratings[hero.name] = hero.rating;
  });
  localStorage.setItem("heroRatings", JSON.stringify(ratings));
}
...
function displayHeroes() {
  ...
  heroes.forEach((hero) => {
    ...
    card.innerHTML = `
    <img class="image" src="${hero.url}" alt="${hero.name}">
          <h2>${hero.name}</h2>
  ...
  document.querySelectorAll(".rating").forEach((ratingDiv) => {
    const heroName = ratingDiv.getAttribute("data-hero");
    ratingDiv.addEventListener("click", (event) => {
      const stars = Array.from(ratingDiv.children);
      const index = stars.indexOf(event.target);
      if (index !== -1) {
        heroes.find((h) => h.name === heroName).rating = index + 1; // Устанавливаем рейтинг
        saveRatings(); // Сохраняем рейтинг
        updateStars(ratingDiv, index + 1); // Обновляем звезды
      }
    });
    updateStars(ratingDiv, heroes.find((h) => h.name === heroName).rating); // Обновляем звезды при загрузке
  });
}
